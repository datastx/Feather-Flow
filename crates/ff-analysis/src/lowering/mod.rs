...
mod tests {
    ...
    fn test_lower_full_outer_join() {
        let catalog = orders_customers_catalog();
        let sql =
            "SELECT o.id, c.name FROM orders o FULL OUTER JOIN customers c ON o.cust_id = c.id";
        let ir = parse_and_lower(sql, &catalog);
    ...
    fn test_lower_delete_fails() {
        let dialect = DuckDbDialect {};
        let stmts =
            Parser::parse_sql(&dialect, "DELETE FROM t WHERE id = 1").expect("parse failed");
        let result = lower_statement(&stmts[0], &empty_catalog());
        assert!(result.is_err());
    }
    ...
    fn test_lower_create_table_fails() {
        let dialect = DuckDbDialect {};
        let stmts =
            Parser::parse_sql(&dialect, "CREATE TABLE t (id INTEGER)").expect("parse failed");
        let result = lower_statement(&stmts[0], &empty_catalog());
        assert!(result.is_err());
    }
