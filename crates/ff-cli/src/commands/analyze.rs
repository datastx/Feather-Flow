...
/// Execute SQL rules against the meta database. Returns true if there were errors.
fn execute_rules(
    ...
    if rules_config.paths.is_empty() {
        println!("\nNo rule paths configured. Add `rules.paths` to featherflow.yml.");
        return Ok(false);
    }
    ...
    if rules.is_empty() {
        println!("\nNo rule files found in configured paths.");
        return Ok(false);
    }
    ...
    if list_only {
        let rows: Vec<Vec<String>> = rules
            ...
            .collect();
        print_common_table(&["NAME", "SEVERITY", "DESCRIPTION", "PATH"], &rows);
        return Ok(false);
    }

    println!("\nRunning {} rules against meta database...\n", rules.len());
